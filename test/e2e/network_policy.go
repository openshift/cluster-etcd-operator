...
var _ = g.Describe("[sig-etcd] cluster-etcd-operator", func() {
	...
	g.It("[Operand][NetworkPolicy] should allow installer and pruner pods to access apiserver", func() {
		...
		g.By("Validating egress to API server port 6443")
		requireEgressPort(policy, corev1.ProtocolTCP, 6443)
		g.GinkgoWriter.Printf("  - Egress port: TCP/6443 (API server)\n")

		g.By("Validating pod selector uses matchExpressions with In operator")
		// Verify podSelector uses matchExpressions for installer and pruner
		...
		for _, expr := range policy.Spec.PodSelector.MatchExpressions {
			if expr.Key == "app" && expr.Operator == metav1.LabelSelectorOpIn {
				...
				g.GinkgoWriter.Printf("  - Installer pods manage etcd static pod manifests\n")
				g.GinkgoWriter.Printf("  - Pruner pods clean up old revision resources\n")
				found = true
				break
			}
		}
		o.Expect(found).To(o.BeTrue(), "should have app matchExpression with installer and pruner")
		g.GinkgoWriter.Printf("=== Installer/Pruner policy structure validated successfully ===\n")

		// Note about connectivity testing
		g.GinkgoWriter.Printf("\n=== Note on Installer/Pruner API Server Connectivity ===\n")
		g.GinkgoWriter.Printf("  - Installer/Pruner pods connect to actual API server endpoint (not kubernetes service)\n")
	...
	g.It("[Operand][NetworkPolicy] should enforce allow rules for etcd operand pods", func() {
		...
		requireEgressPort(ipPolicy, corev1.ProtocolTCP, 6443)
		o.Expect(ipPolicy.Spec.PodSelector.MatchExpressions).NotTo(o.BeEmpty(), "should use matchExpressions")
		found := false
		...
		o.Expect(found).To(o.BeTrue(), "should have app matchExpression with installer and pruner")
		g.GinkgoWriter.Printf("  - allow-installer-pruner-to-apiserver: app in (installer, pruner) → TCP/6443\n")
...
func validateEtcdNamespacePolicies(ctx context.Context, client kubernetes.Interface) {
	...
	g.GinkgoWriter.Printf("  Checking for API server egress (TCP/6443)...\n")
