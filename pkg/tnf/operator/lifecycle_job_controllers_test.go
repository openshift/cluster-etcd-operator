...
├── TestGetNodesFromLatestUpdateSetupConfigMap - Update-setup job node list validation
│   ├── Normal case - all ConfigMap nodes exist in cluster
│   ├── Stale ConfigMap - node missing from cluster (error)
│   ├── Node replaced - different UID (error)
│   ├── No ConfigMaps exist (empty result)
│   ├── Latest ConfigMap selected by highest generation
│   └── Invalid JSON in ConfigMap (error)
...
func TestStartJobControllers(t *testing.T) {
	tests := []struct {
		...
		{
			name:               "after transition - 0 nodes - skip",
			transitionComplete: true,
			nodeCount:          0,
			readyNodeCount:     0,
			expectStartCalled:  false,
...
// TestGetNodesFromLatestUpdateSetupConfigMap tests the ConfigMap parsing logic
// that determines which nodes an update-setup job should affect.
// This is critical for safety - ensures we don't run jobs on stale node lists.
func TestGetNodesFromLatestUpdateSetupConfigMap(t *testing.T) {
	tests := []struct {
		name           string
		...
		{
			name: "normal case - all ConfigMap nodes exist",
			configMaps: []*corev1.ConfigMap{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "tnf-update-setup-5",
						Namespace: operatorclient.TargetNamespace,
						Labels: map[string]string{
							"app.kubernetes.io/component": "tnf-update-setup",
						},
					},
					Data: map[string]string{
						"generation": "5",
						"nodes":      `[{"name":"master-0","ip":"10.0.0.1","uid":"uid-0"},{"name":"master-1","ip":"10.0.0.2","uid":"uid-1"}]`,
					},
				},
			},
			nodes: []*corev1.Node{
		...
		{
			name: "stale ConfigMap - node in ConfigMap but not in cluster",
			configMaps: []*corev1.ConfigMap{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "tnf-update-setup-5",
			...
			nodes: []*corev1.Node{
				{ObjectMeta: metav1.ObjectMeta{Name: "master-0", UID: "uid-0"}},
				// master-1 missing (deleted/replaced)
			},
			expectedNodes: nil,
			expectError:   true,
			errorContains: "references nodes not found in cluster",
		},
		{
			name: "node replaced - different UID",
			configMaps: []*corev1.ConfigMap{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "tnf-update-setup-5",
					...
					Data: map[string]string{
						"generation": "5",
						"nodes":      `[{"name":"master-0","ip":"10.0.0.1","uid":"uid-0-old"}]`,
					},
				},
			},
			nodes: []*corev1.Node{
				{ObjectMeta: metav1.ObjectMeta{Name: "master-0", UID: "uid-0-new"}},
			},
			expectedNodes: nil,
			expectError:   true,
			errorContains: "references nodes not found in cluster",
		...
		{
			name: "latest ConfigMap selected by highest generation",
			configMaps: []*corev1.ConfigMap{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "tnf-update-setup-3",
						Namespace: operatorclient.TargetNamespace,
						Labels: map[string]string{
							"app.kubernetes.io/component": "tnf-update-setup",
						},
					...
					Data: map[string]string{
						"generation": "3",
						"nodes":      `[{"name":"master-0","ip":"10.0.0.1","uid":"uid-0"}]`,
					},
				},
				{
					ObjectMeta: metav1.ObjectMeta{
					...
					Data: map[string]string{
						"generation": "5",
						"nodes":      `[{"name":"master-1","ip":"10.0.0.2","uid":"uid-1"}]`,
					},
				},
			},
			nodes: []*corev1.Node{
			...
			expectedNodes: []string{"master-1"}, // Only gen 5 node
			expectError:   false,
		},
		{
			...
			expectedNodes: nil,
			expectError:   true,
			errorContains: "failed to unmarshal nodes",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create node informer and add nodes manually
			nodeIndexer := cache.NewIndexer(cache.MetaNamespaceKeyFunc, cache.Indexers{})
			...
			// Add ConfigMaps manually to the informer's indexer AFTER sync
			...
			for _, cm := range tt.configMaps {
				require.NoError(t, configMapInformer.GetIndexer().Add(cm))
			}

			// Execute
			result, err := getNodesFromLatestUpdateSetupConfigMap(nodeInformer, kubeInformersForNamespaces)
