...
var (
	...
	// Per wave: exponential backoff, then Degraded + checkpoint (OCPBUGS-84336).
	...
	// Between waves after backoff exhaustion (OCPBUGS-84336); tests shorten.
...
func tryStartTnfJobControllers(
	...
	// Launch once unconditionally; readiness, labels, and revision stability are
	// validated with polling in handleNodes (waitForTnfControlPlaneReadyAndStableRevision).
	once.Do(func() {
		go handleNodesWithRetry(nodes, ctx, controllerContext, operatorClient, kubeClient, kubeInformersForNamespaces, etcdInformer)
	})
}
...
func handleNodesWithRetry(
	...
	// Exponential backoff per wave; Degraded + checkpoint on exhaustion (OCPBUGS-84336).
	for {
		if ctx.Err() != nil {
...
// waitForTnfControlPlaneReadyAndStableRevision polls 10s/10m until nodeList are Ready with control-plane labels and IsRevisionStable (OCPBUGS-84336).
	...
	return wait.PollUntilContextTimeout(ctx, interval, timeout, false, func(ctx context.Context) (bool, error) {
		for _, ref := range nodeList {
			...
			if err != nil {
				...
				klog.Warningf("TNF gate: node %q API error (will retry): %v", ref.Name, err)
				return false, nil
			}
			if !hasControlPlaneRoleLabels(n) {
		...
		if err != nil {
			klog.Warningf("IsRevisionStable check failed (will retry): %v", err)
			return false, nil
		}
		if !stable {
...
func handleFencingSecretChange(ctx context.Context, operatorClient v1helpers.StaticPodOperatorClient, client kubernetes.Interface, oldObj, obj interface{}) {
	...
	// Defer fencing job restart from secrets until external etcd transition completes (OCPBUGS-84336).
	...
	if !transitionComplete {
		klog.Infof("skipping fencing job restart from fencing secret %s change until %s is True",
			secret.GetName(), etcd.OperatorConditionExternalEtcdHasCompletedTransition)
		return
	}
