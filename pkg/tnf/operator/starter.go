...
func handleFencingSecretChange(ctx context.Context, client kubernetes.Interface, oldObj, obj interface{}) {
	...
	if oldObj != nil {
		...
		if len(oldSecret.Data) == len(secret.Data) {
			for key, oldValue := range oldSecret.Data {
				if !bytes.Equal(oldValue, secret.Data[key]) {
					changed = true
					break
				}
			}
	...
	// TODO make all job timeouts constants and put them into a central place for easier modification
	// wait as long as the fencing job waits as well, plus some execution time
	err := wait.PollUntilContextTimeout(ctx, 15*time.Second, 15*time.Minute, true, isFencingJobRunning)
	if err != nil {
