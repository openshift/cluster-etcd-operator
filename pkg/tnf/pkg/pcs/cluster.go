...
func ConfigureCluster(ctx context.Context, cfg config.ClusterConfig) (bool, error) {
	...
	commands := []string{
		...
		// Note: Setting low `migration-threshold` to prevent endless restart loops caused by the infinite default. This provides a safe limit on retries.
		"/usr/sbin/pcs resource create kubelet systemd:kubelet op start timeout=180s start-delay=5s clone meta interleave=true migration-threshold=15",
