...
func TestGetStonithCommand(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: `/usr/sbin/pcs stonith create node1_redfish fence_redfish username="admin" password="pass123" ip="192.168.111.1" ipport="8000" systems_uri="redfish/v1/Systems/abc" pcmk_host_list="node1"`,
		},
		{
			...
			want: `/usr/sbin/pcs stonith create node2_redfish fence_redfish username="admin" password="pass123" ip="192.168.111.2" ipport="8000" systems_uri="redfish/v1/Systems/def" pcmk_host_list="node2" ssl_insecure="1"`,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := getStonithCommand(tt.fc)
			if got != tt.want {
