...
func TestHealthCheck_eventDeduplication_DifferentWindows(t *testing.T) {
	...
	// Record a resource action warning (transition from Unknown to Warning records PacemakerHealthy)
	...
	initialEvents := len(recorder.Events())
	require.Equal(t, 2, initialEvents, "Should have recorded warning event + PacemakerHealthy transition event")

	// Record a fencing event warning (staying in Warning, no transition event)
	fencingStatus := &HealthStatus{
		OverallStatus: statusWarning,
		Warnings: []string{
	...
	controller.recordHealthCheckEvents(fencingStatus)
	afterFencingEvents := len(recorder.Events())
	require.Equal(t, 3, afterFencingEvents, "Should have recorded fencing event")

	// Verify both warning events are tracked separately (+ healthy transition is not deduplicated)
	controller.recordedEventsMu.Lock()
	require.Len(t, controller.recordedEvents, 2, "Should have 2 warning events in deduplication map")
	controller.recordedEventsMu.Unlock()

	// Try to record the same events again - warning events should be deduplicated
	controller.recordHealthCheckEvents(resourceActionStatus)
	controller.recordHealthCheckEvents(fencingStatus)
	afterDuplicates := len(recorder.Events())
	require.Equal(t, afterFencingEvents, afterDuplicates, "Duplicate warning events should not be recorded")
}
