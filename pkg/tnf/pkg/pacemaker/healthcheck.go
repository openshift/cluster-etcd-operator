...
const (
	...
	errorSubstringNodeUnhealthy      = "node is unhealthy"
	errorSubstringClusterUnhealthy   = "Cluster is unhealthy"
...
func (c *HealthCheck) getPacemakerStatus(ctx context.Context) (*HealthStatus, *HealthStatus, error) {
	...
	pacemakerCR := &v1alpha1.PacemakerCluster{}
	...
	if err != nil {
		...
		return &HealthStatus{
			...
			Warnings:      []string{},
			Errors:        []string{fmt.Sprintf("Failed to get PacemakerCluster CR: %v", err)},
		}, previous, nil
	}
...
func (c *HealthCheck) getEventReasonForError(errorMsg string) string {
	switch {
...
func (c *HealthCheck) recordErrorEvent(errorMsg string) {
	eventReason := c.getEventReasonForError(errorMsg)
