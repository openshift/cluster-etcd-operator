...
const (
	...
	errorSubstringNodeUnhealthy      = "is unhealthy"
	errorSubstringNodeOffline        = "is offline"
	...
	errorSubstringInsufficientNodes  = "Insufficient nodes"
	errorSubstringExcessiveNodes     = "Excessive nodes"
	errorSubstringClusterMaintenance = "maintenance mode"
...
func (c *HealthCheck) buildHealthStatusFromCR(pacemakerStatus *v1alpha1.PacemakerCluster) *HealthStatus {
	...
	// Check cluster-level conditions
...
// checkClusterConditions checks cluster-level conditions
func (c *HealthCheck) checkClusterConditions(pacemakerStatus *v1alpha1.PacemakerCluster, status *HealthStatus) {
	...
	if inServiceCondition != nil && inServiceCondition.Status != metav1.ConditionTrue {
		status.Errors = append(status.Errors, msgClusterInMaintenance)
	}

	// Check overall Healthy condition
	...
	if healthyCondition != nil && healthyCondition.Status != metav1.ConditionTrue {
		// Only add if we haven't already identified specific issues above
		if len(status.Errors) == 0 {
			status.Errors = append(status.Errors, fmt.Sprintf(msgClusterUnhealthy, healthyCondition.Message))
		}
	}
}
...
func (c *HealthCheck) checkNodeStatuses(pacemakerStatus *v1alpha1.PacemakerCluster, status *HealthStatus) {
	...
	// Check each node's conditions and resource health
	for _, node := range nodes {
		// Check node-level conditions
		c.checkNodeConditions(node, status)

		// Check resource-level conditions
		c.checkNodeResources(node, status)
	}
}

// checkNodeConditions checks the conditions of a single node
func (c *HealthCheck) checkNodeConditions(node v1alpha1.PacemakerClusterNodeStatus, status *HealthStatus) {
	conditions := node.Conditions
	if len(conditions) == 0 {
		klog.V(2).Infof("Node %s has no conditions", node.Name)
		return
	}
	...
	if healthyCondition != nil && healthyCondition.Status != metav1.ConditionTrue {
		...
		reasons := c.getNodeUnhealthyReasons(conditions, node.Name)
		if len(reasons) > 0 {
			for _, reason := range reasons {
				status.Errors = append(status.Errors, reason)
			}
		} else {
			status.Errors = append(status.Errors, fmt.Sprintf(msgNodeUnhealthy, node.Name, healthyCondition.Message))
		}
	}
}

// getNodeUnhealthyReasons returns specific reasons why a node is unhealthy
func (c *HealthCheck) getNodeUnhealthyReasons(conditions []metav1.Condition, nodeName string) []string {
	var reasons []string

	// Check InService - maintenance mode is unexpected
	inServiceCondition := findCondition(conditions, v1alpha1.NodeInServiceConditionType)
	if inServiceCondition != nil && inServiceCondition.Status != metav1.ConditionTrue {
		reasons = append(reasons, fmt.Sprintf("Node %s is in maintenance mode", nodeName))
	}

	// Check Active - standby mode is unexpected
	activeCondition := findCondition(conditions, v1alpha1.NodeActiveConditionType)
	if activeCondition != nil && activeCondition.Status != metav1.ConditionTrue {
		reasons = append(reasons, fmt.Sprintf("Node %s is in standby mode", nodeName))
	}

	// Check Clean - unclean state indicates fencing/communication issues
	cleanCondition := findCondition(conditions, v1alpha1.NodeCleanConditionType)
	if cleanCondition != nil && cleanCondition.Status != metav1.ConditionTrue {
		reasons = append(reasons, fmt.Sprintf("Node %s is in unclean state (fencing/communication issue)", nodeName))
	}

	// Check Member - not being a member is unexpected
	memberCondition := findCondition(conditions, v1alpha1.NodeMemberConditionType)
	if memberCondition != nil && memberCondition.Status != metav1.ConditionTrue {
		reasons = append(reasons, fmt.Sprintf("Node %s is not a cluster member", nodeName))
	}

	// Ready/Pending is a warning, not an error - it's temporary
	readyCondition := findCondition(conditions, v1alpha1.NodeReadyConditionType)
	if readyCondition != nil && readyCondition.Status != metav1.ConditionTrue {
		klog.V(2).Infof("Node %s is pending (temporary state)", nodeName)
	}

	return reasons
}

// checkNodeResources checks the resource conditions for a node
func (c *HealthCheck) checkNodeResources(node v1alpha1.PacemakerClusterNodeStatus, status *HealthStatus) {
	if len(node.Resources) == 0 {
		klog.V(2).Infof("Node %s has no resources", node.Name)
		return
	}
	...
	for _, resource := range node.Resources {
		...
		if healthyCondition != nil && healthyCondition.Status != metav1.ConditionTrue {
			// Get specific reason for unhealthy resource
			reason := c.getResourceUnhealthyReason(resource, node.Name)
			status.Errors = append(status.Errors, reason)
		}
	}
}

// getResourceUnhealthyReason returns a specific reason why a resource is unhealthy
func (c *HealthCheck) getResourceUnhealthyReason(resource v1alpha1.PacemakerClusterResourceStatus, nodeName string) string {
	...
	if operationalCondition != nil && operationalCondition.Status != metav1.ConditionTrue {
		return fmt.Sprintf(msgResourceUnhealthy, resource.Name, nodeName, "resource has failed")
	}
	...
	if startedCondition != nil && startedCondition.Status != metav1.ConditionTrue {
		return fmt.Sprintf(msgResourceUnhealthy, resource.Name, nodeName, "resource is stopped")
	}

	activeCondition := findCondition(conditions, v1alpha1.ResourceActiveConditionType)
	if activeCondition != nil && activeCondition.Status != metav1.ConditionTrue {
		return fmt.Sprintf(msgResourceUnhealthy, resource.Name, nodeName, "resource is not active")
	}
	...
	if managedCondition != nil && managedCondition.Status != metav1.ConditionTrue {
		return fmt.Sprintf(msgResourceUnhealthy, resource.Name, nodeName, "resource is unmanaged")
	}

	// Generic unhealthy message
