...
func buildStatusComponents(result *PacemakerResult, clusterConfig *ClusterConfig) (
	...
	if clusterConfig != nil && len(clusterConfig.Nodes) > 0 {
		...
		// Skip if required Name field is missing
		if configNode.Name == "" {
			klog.Warningf("Skipping node with empty name from cluster config")
			continue
		}

		// Always create node status, even if some optional fields are missing
	...
	processResource := func(resource Resource) {
		...
		if resource.ID == "" {
			klog.Warningf("Skipping resource with empty ID")
			return
		}

		// Node field is optional - stopped resources won't have a node assignment
		nodeName := resource.Node.Name
		if nodeName == "" {
			klog.V(4).Infof("Resource '%s' has no node assignment (likely stopped)", resource.ID)
		}
	...
	for _, node := range result.NodeHistory.Node {
		for _, resourceHistory := range node.ResourceHistory {
			for _, operation := range resourceHistory.OperationHistory {
				// Skip if required fields are missing
				if node.Name == "" {
					klog.Warningf("Skipping node history entry with empty node name")
					continue
				}
				if resourceHistory.ID == "" {
					klog.Warningf("Skipping node history entry for node '%s' with empty resource ID", node.Name)
					continue
				}
				if operation.Task == "" {
					klog.Warningf("Skipping node history entry for node '%s', resource '%s' with empty operation", node.Name, resourceHistory.ID)
					continue
				}
				...
				if err != nil {
					klog.Warningf("Skipping node history entry due to failed timestamp parse: %v", err)
					continue
				}
	...
	for _, fenceEvent := range result.FenceHistory.FenceEvent {
		// Skip if required Target field is missing
		if fenceEvent.Target == "" {
			klog.Warningf("Skipping fencing event with empty target")
			continue
		}
		...
		if err != nil {
			klog.Warningf("Skipping fencing event for target '%s' due to failed timestamp parse: %v", fenceEvent.Target, err)
			continue
		}
...
func updatePacemakerStatusCR(
	...
	if !existing.Status.LastUpdated.IsZero() && !now.After(existing.Status.LastUpdated.Time) {
		klog.Warningf("Skipping update: our timestamp (%s) is not newer than existing timestamp (%s). This likely means another collector job has already updated the status.",
			now.Format(time.RFC3339), existing.Status.LastUpdated.Format(time.RFC3339))
		return nil
	}
