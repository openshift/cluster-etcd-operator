...
func TestBuildStatusComponents_StoppedResourceHandling(t *testing.T) {
	// Test that stopped resources (no node assignment) are included with sentinel value
	result := &PacemakerResult{
		Summary: Summary{
			Stack:     Stack{PacemakerdState: "running"},
			CurrentDC: CurrentDC{WithQuorum: "true"},
			...
			Clone: []Clone{
				{
					Resource: []Resource{
						...
						},
						{
							...
							Role:          "Stopped",
							Active:        "false",
							Node:          NodeRef{Name: ""}, // Stopped resources have no node
						},
					},
				},
			},
	...
	// Should have both resources (started and stopped)
	require.Len(t, resources, 2, "Both started and stopped resources should be included")

	// Find the stopped resource
	var stoppedResource *v1alpha1.PacemakerResourceStatus
	var startedResource *v1alpha1.PacemakerResourceStatus
	for i := range resources {
		if resources[i].Role == v1alpha1.ResourceRoleStopped {
			stoppedResource = &resources[i]
		} else if resources[i].Role == v1alpha1.ResourceRoleStarted {
			startedResource = &resources[i]
		}
	}

	require.NotNil(t, stoppedResource, "Should find stopped resource")
	require.NotNil(t, startedResource, "Should find started resource")

	// Verify stopped resource has empty node value (field is optional)
	require.Empty(t, stoppedResource.Node, "Stopped resource should have empty node value")
	require.Equal(t, v1alpha1.ResourceRoleStopped, stoppedResource.Role)
	require.Equal(t, v1alpha1.ResourceActiveStatusInactive, stoppedResource.ActiveStatus)

	// Verify started resource has actual node
	require.Equal(t, "master-0", startedResource.Node)
	require.Equal(t, v1alpha1.ResourceRoleStarted, startedResource.Role)
	require.Equal(t, v1alpha1.ResourceActiveStatusActive, startedResource.ActiveStatus)
}
