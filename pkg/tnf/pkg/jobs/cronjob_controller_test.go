...
func TestPtrHelpers(t *testing.T) {
	...
	t.Run("ptrBoolValue", func(t *testing.T) {
		if ptrBoolValue(nil) != false {
			t.Error("ptrBoolValue(nil) should return false")
		}
		trueVal := true
		if ptrBoolValue(&trueVal) != true {
			t.Error("ptrBoolValue(&true) should return true")
		}
	})

	t.Run("ptrInt32Value", func(t *testing.T) {
		if ptrInt32Value(nil) != 0 {
			t.Error("ptrInt32Value(nil) should return 0")
		}
		val := int32(42)
		if ptrInt32Value(&val) != 42 {
			t.Error("ptrInt32Value(&42) should return 42")
		}
	})
}
...
func TestNeedsUpdate_DeepEqualityAfterNormalization(t *testing.T) {
	...
	if controller.needsUpdate(existing, desired) {
		...
		if !equality.Semantic.DeepEqual(existingCopy.Spec, desiredCopy.Spec) {
			t.Error("Specs should be equal after normalization but needsUpdate returned true")
		}
	}
}
