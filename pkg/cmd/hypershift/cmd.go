...
func NewHypershiftOperator() *cobra.Command {
	opts := &Options{
		etcdEndpoints: []string{"https://localhost:2379"},
	}

	cmd := controllercmd.
		NewControllerCommandConfig("hypershift-etcd-operator", version.Get(), opts.runOperator).
		NewCommandWithContext(context.Background())
	cmd.Use = "operator"
	cmd.Short = "Start the Hypershift etcd Operator"
...
func (r *Options) runOperator(ctx context.Context, controllerContext *controllercmd.ControllerContext) error {
...
func (r *Options) etcdEndpointsFunc() ([]string, error) {
