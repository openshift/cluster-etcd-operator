...
import (
	...
	"github.com/openshift/cluster-etcd-operator/pkg/tnf-operator/exec"
)
...
func ConfigureEtcd(ctx context.Context, cfg config.ClusterConfig) error {
	...
	if !strings.Contains(stdOut, "etcd") {
		...
		cmd := fmt.Sprintf("/usr/sbin/pcs resource create etcd podman-etcd image=%s authfile=/etc/podman/pull-secret node_ip_map=\"%s:%s;%s:%s\" drop_in_dependency=true clone meta interleave=true notify=true",
			cfg.EtcdPullSpec, cfg.NodeName1, cfg.NodeIP1, cfg.NodeName2, cfg.NodeIP2)
