...
function dl_etcdctl {
  # avoid caching the binary when podman exists, the  etcd image is always available locally and we need a way to update etcdctl if necessary
  # when we're running from an etcd image, there's no podman (yet an updated etcdctl) and we can continue without download
