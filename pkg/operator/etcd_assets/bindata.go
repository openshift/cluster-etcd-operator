    ...
    - name: fio-benchmark
      ...
      command:
        ...
        - |
          ...
          mkdir -p /etc/kubernetes/static-pod-resources/benchmark

          # run once
          if [ -f /etc/kubernetes/static-pod-resources/benchmark/storage-benchmark-results.json ]; then
            exit 0
          fi

          # based on https://www.ibm.com/cloud/blog/using-fio-to-tell-whether-your-storage-is-fast-enough-for-etcd
          fio \
            --rw=write \
            --ioengine=sync \
            --fdatasync=1 \
            --size=22m \
            --bs=2300 \
            --name=storage-benchmark-workfile \
            --directory=/etc/kubernetes/static-pod-resources/benchmark \
            --output-format=json \
            --output /etc/kubernetes/static-pod-resources/benchmark/storage-benchmark-results.json
         
          # POC probably should be done in go for flexibility.
          BENCHMARK=($(cat /etc/kubernetes/static-pod-resources/benchmark/storage-benchmark-results.json \
            | jq '.jobs[0].sync.lat_ns.percentile | ."99.000000",."99.990000"'))

          # we would write something like this to disk which would be consumed by node_exporter
          echo "# HELP benchmark_fsync_duration_seconds reports on storage latency during cluster install"
          echo "# TYPE benchmark_fsync_duration_seconds summary"
          echo "benchmark_fsync_duration_seconds{quantile=\"0.99\"} ${BENCHMARK[0]}"
          echo "benchmark_fsync_duration_seconds{quantile=\"0.9999\"} ${BENCHMARK[1]}"

          # cleanup workfile  
          rm -f /etc/kubernetes/static-pod-resources/benchmark/storage-benchmark-workfile.0.0
