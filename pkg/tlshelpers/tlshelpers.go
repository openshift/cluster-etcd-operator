...
func CreateSignerCert(
	...
	signerValidity CertValidity) certrotation.RotatedSigningCASecret {
	...
	return certrotation.RotatedSigningCASecret{
		...
		AdditionalAnnotations: certrotation.AdditionalAnnotations{
			...
			Description: fmt.Sprintf("Generated by cluster-etcd-operator for etcd and is used to sign peer, server and client certificates. "+
				"This certificate is valid for %d days and starts refreshing after %d days.", durationDays(signerValidity.Validity), durationDays(signerValidity.RefreshAt)),
		},
		Validity: signerValidity.Validity,
		Refresh:  signerValidity.RefreshAt,
...
func CreateBootstrapSignerCert(
	...
	secretGetter corev1client.SecretsGetter,
	recorder events.Recorder,
	certValidity CertValidity) certrotation.RotatedSigningCASecret {
	secret := CreateSignerCert(secretInformer, secretLister, secretGetter, recorder, certValidity)
...
func CreateBootstrapMetricsSignerCert(
	...
	recorder events.Recorder,
	certValidity CertValidity) certrotation.RotatedSigningCASecret {
	secret := CreateMetricsSignerCert(secretInformer, secretLister, secretGetter, recorder, certValidity)
	secret.Namespace = operatorclient.GlobalUserSpecifiedConfigNamespace
	return secret
}
...
func CreatePeerCertificate(node *corev1.Node,
	...
	secretGetter corev1client.SecretsGetter,
	recorder events.Recorder,
	certValidity CertValidity) (*certrotation.RotatedSelfSignedCertKeySecret, error) {
	return createCertForNode(
		fmt.Sprintf("Peer (client and server) certificate for node %s, generated by cluster-etcd-operator for etcd. "+
			"This certificate is valid for %d days and starts refreshing after %d days.", node.Name, durationDays(certValidity.Validity), durationDays(certValidity.RefreshAt)),
		GetPeerClientSecretNameForNode(node.Name),
		node, secretInformer, secretLister, secretGetter, recorder, certValidity)
}
...
func createCertForNode(description, secretName string, node *corev1.Node,
	...
	return &certrotation.RotatedSelfSignedCertKeySecret{
		...
		Validity:    certValidity.Validity,
		Refresh:     certValidity.RefreshAt,
		CertCreator: creator,
...
func CreateMetricsClientCert(
	...
	secretGetter corev1client.SecretsGetter,
	recorder events.Recorder,
	certValidity CertValidity) certrotation.RotatedSelfSignedCertKeySecret {
	creator := &CARotatingTargetCertCreator{
	...
	return certrotation.RotatedSelfSignedCertKeySecret{
		...
		AdditionalAnnotations: certrotation.AdditionalAnnotations{
			...
			Description: fmt.Sprintf("Client certificate for Prometheus ServiceMonitors to reach etcd grpc-proxy to retrieve metrics. "+
				...
				"This certificate is valid for %d days and starts refreshing after %d days.", durationDays(certValidity.Validity), durationDays(certValidity.RefreshAt)),
		},
		Validity:    certValidity.Validity,
		Refresh:     certValidity.RefreshAt,
		CertCreator: creator,
