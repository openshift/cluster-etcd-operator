...
var map_InfrastructureStatus = map[string]string{
	...
	"infrastructureTopology": "InfrastructureTopology expresses the expectations for operands that normally run on infrastructure nodes. The default is 'HighlyAvailable', which represents the behavior operators have in a \"normal\" cluster. The 'SingleReplica' mode will be used in single-node deployments (developer and production) for example, and the operators should not configure the operand for highly-available operation",
}
