...
func ApplyServiceMonitor(client dynamic.Interface, shouldDelete bool, recorder events.Recorder, required *unstructured.Unstructured) (*unstructured.Unstructured, bool, error) {
	...
	if shouldDelete {
		err := client.Resource(serviceMonitorGVR).Namespace(namespace).Delete(context.TODO(), existing.GetName(), metav1.DeleteOptions{})
		if err != nil {
			return nil, false, err
		}
...
func ApplyPrometheusRule(client dynamic.Interface, shouldDelete bool, recorder events.Recorder, required *unstructured.Unstructured) (*unstructured.Unstructured, bool, error) {
	namespace := required.GetNamespace()
	...
	if shouldDelete {
		err := client.Resource(prometheusRuleGVR).Namespace(namespace).Delete(context.TODO(), existing.GetName(), metav1.DeleteOptions{})
		if err != nil {
			return nil, false, err
		}
		reportDeleteEvent(recorder, required, err)
